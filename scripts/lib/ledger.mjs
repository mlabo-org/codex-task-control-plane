...
function migrateTask(task, pathExists) {
  ...
  task.git = isRecord(task.git) ? task.git : { commonDirectory: null, primaryCheckout: task.cwd || null, targetBranch: task.target.integrationTargetBranch || null, targetHeadAtDispatch: null, accessMode };
  task.worktree = isRecord(task.worktree) ? task.worktree : { managed, purpose: task.target.worktreePurpose, authority: task.target.worktreeLifecycleAuthority, runtimeCwd, threadId: task.threadId || null, clientThreadId: task.clientThreadId || null, pinned: false, headAtReview: null, branchAtReview: null, candidateFingerprint: null };
  task.settlement = managed ? { required: true, decision: prior.decision || null, phase: terminal ? (existing ? "orphan_recovery_required" : "cleanup_pending") : "awaiting_decision", terminalStatus: terminal ? task.status : null, operationIds: Array.isArray(prior.operationIds) ? prior.operationIds : [], adoptionReceipt: prior.adoptionReceipt || null, cleanupReceipt: prior.cleanupReceipt || null, blocker: prior.blocker || null } : { required: false, decision: null, phase: "not_required", terminalStatus: null, operationIds: [], adoptionReceipt: null, cleanupReceipt: null, blocker: null };
